...
export default function TrainingContentPageInner() {
  ...
  const fetchTrainingVideos = async () => {
    ...
    try {
      ...
      setVideos(data.sort((a: Resource, b: Resource) => a.order - b.order));
      setError(null);
    } catch (err) {
  ...
  const fetchTrainingDocuments = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch resources");
      const data = await response.json();
      setDocuments(data.sort((a: Resource, b: Resource) => a.order - b.order));
      setError(null);
    } catch (err) {
      setError("Error loading resources");
      console.error(err);
